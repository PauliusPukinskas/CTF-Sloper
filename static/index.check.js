...
function overviewHtml(sum){ return `<h3>Best flags</h3>${flagsHtml(sum, 8)}<h3>Clean artifact queue</h3>${artifactsHtml((sum.artifacts||[]).slice(0,12))}`; }
...
function flagRow(f, weak){ const val = f.preferred_flag || f.flag || f.value || ''; return `<div class="flag"><div class="row between"><b class="${weak?'warn':'ok'}">${esc(val)}</b><button class="btn" onclick="copyText('${esc(val)}')">copy</button></div><div class="sub">score ${esc(f.score||0)} · ${esc(f.source||'')} · ${esc(f.file||'')}</div></div>`; }
