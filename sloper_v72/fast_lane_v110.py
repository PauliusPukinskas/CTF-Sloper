...
def scan_text(text: str, profile: dict[str, Any] | None = None, raw: bytes | None = None) -> list[dict[str, Any]]:
    ...
    # v114 specialized extractors handle heavyweight binary containers better than
    # the generic text decoder. Keep the fast lane shallow for these formats so
    # SQLite/WAV/PNG/DOCX files do not explode into useless XOR/ROT candidates.
    if raw.startswith((b"SQLite format 3\x00", b"RIFF", b"\x89PNG\r\n\x1a\n", b"PK\x03\x04")):
    ...
    while queue and processed < max_nodes:
        ...
        # Single-byte XOR only on small text-ish nodes to keep UI responsive.
        # v114 has a separate XOR rescue for binary payloads.
        if len(blob) <= 120_000 and (_printable_ratio(blob) >= 0.25 or b"{" in blob or b"}" in blob):
